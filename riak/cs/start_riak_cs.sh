...
until [ "$A" = "" ]; do
    echo "Waiting for KV"
    B=$(curl -s -XPOST -s $RIAK_IP:$RIAK_PORT/buckets/waiting4kv/counters/$IP -d $RANDOM) && \
    A=$B
done
...
if [ $BOOTSTRAP -eq 1 ] && \
    [ "$(curl -s $RIAK_IP:$RIAK_PORT/buckets/admin/keys/secret)" = 'not found' ]; then
    ...
    curl -s -XPUT $RIAK_IP:$RIAK_PORT/buckets/admin/keys/key \
        -d "$(echo $KEY | openssl enc -a -e -aes-256-cbc -nosalt -pass pass:$PASSWD)"
    curl -s -XPUT $RIAK_IP:$RIAK_PORT/buckets/admin/keys/secret \
